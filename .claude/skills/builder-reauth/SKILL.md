...
2. **Relay the click-path verbatim.** The helper prints the exact steps; pass them to the user clearly (don't paraphrase the URL):
   - Open `https://amazonprimea.com/builders/settings` (the helper tries to open it automatically; if not, give them the link).
...
- **"You are not signed in" / `cookie_required`.** The re-issue endpoint mints CLI bearers from a browser *cookie*, not a bearer — a leaked CLI token can't extend its own life. If the Settings page shows them logged out, they sign in at `https://amazonprimea.com/builders` first, then return to `/builders/settings` and click Re-issue. Once signed in the button works.
...
- **`GDS_API_BASE`** overrides the endpoint (default `https://amazonprimea.com`); for local dev `GDS_API_BASE=http://localhost:3000`.
